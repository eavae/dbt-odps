...
class TestConnectionOdpsAdapter(unittest.TestCase):
    ...
    def test_quoting_on_drop(self):
        ...
        self.mock_execute.assert_has_calls(
            [
                mock.call(
                    "drop table if exists `mock_project`.`test_table`",
                    None,
                )
            ]
        )

    def test_list_relations_without_cache(self):
        self.cursor.description = [("_c0",)]
        self.cursor.fetchall.return_value = [
            ("1972049018142919:raw_customers\n1972049018142919:raw_orders\n1972049018142919:raw_payments\n",)
        ]
        ...
        assert [str(r) for r in relations] == [
            "`mock_project`.`raw_customers`",
            "`mock_project`.`raw_orders`",
            "`mock_project`.`raw_payments`",
        ]
