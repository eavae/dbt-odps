...
{% macro odps__check_schema_exists(information_schema,schema) -%}
...
{% macro odps__create_schema(relation) -%}
...
{% macro odps__drop_schema(relation) -%}
...
{% macro odps__get_columns_in_relation(relation) -%}
...
{% macro odps__list_relations_without_caching(schema_relation) -%}
'''creates a table of relations withough using local caching.'''
{% endmacro %}

{% macro odps__list_schemas(database) -%}
